...
- Added strict, presence-aware output selection with source-bound saved choices, explicit theme clearing, config-relative asset origins, and immutable JS/CSS bundles. Theme-byte changes rebuild EPUB output without model calls; TXT retains EPUB selections without reading presentation assets or running JavaScript.
- Integrated the same bounded classifier and exact-address CSS service into source-backed and generated mono/bilingual EPUB assembly, with protected-scope checks, inline-priority admission, and preflight before paid work.
- Publication now independently verifies every requested EPUB and reverses only admitted theme changes before preservation checks, without rerunning JavaScript. Files publish sequentially with durable partial-success receipts, not multi-file atomicity. Actual source/mono/bilingual triplet evidence is persisted and bound to current file hashes and the output digest for benchmark admission; inherited structural diagnostics are not a new production publication gate.
- Release CI's standalone QuickJS/tinycss2 probes and actual frozen application smoke passed on all five targets: Linux x64/ARM64, macOS Intel/Apple Silicon, and Windows x64. Each executable assembled source-backed/generated mono/bilingual EPUBs with an empty child `PATH`; no tag or GitHub Release was created.
- Local Black Swan before/after EPUBs were assembled through the actual CLI in a deny-network sandbox from copied completed state; source, translation-target, usage and publication hashes remained unchanged after reader checks. All 827 offline tests passed. Calibre's local Content-server reader was checked in 24 fixed-width light/dark views, with inherited dark-table contrast and live-resize limitations recorded separately. Desktop Calibre and WeRead are not claimed as verified; WeRead upload was declined.
