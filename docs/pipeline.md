...
- **Cross-chunk arbitration:** after all concurrent chunks finish, contradictory consistency proposals for the same term, pronoun, or fixed expression can be sent through a final arbiter. The final suggestion set conservatively rewrites every losing proposal to the winning value; every superseded proposal remains available in the round traces. Unresolved groups are withheld from actionable final issues, exposed separately in `result.json`, and retained with full evidence under `rounds/final/unresolved_issues.json` for manual review. It never changes the glossary or translated text.
