...
- **Final review:** starts only after every chapter has been translated, so each chapter derives its relevant term snapshot from the completed glossary rather than the glossary state from an earlier chapter. Chapters are divided into contiguous chunks and checked in parallel against fixed final translation and term snapshots; results are merged back in book order. Only terms found in a chunk's source text are sent to its reviewer. Terminology findings receive a second, adversarial pass that treats the model-generated glossary as fallible evidence and removes a finding only when the challenger explicitly rejects it; malformed or incomplete challenger output keeps the first-pass finding. Severe issues are only retranslated when `autofix_severe` is enabled.
