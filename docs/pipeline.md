...
- **Final review:** starts only after every chapter has been translated, so each chapter derives its relevant term snapshot from the completed glossary rather than the glossary state from an earlier chapter. Chapters are divided into contiguous chunks and checked in parallel against fixed final translation and term snapshots; results are merged back in book order. If a reviewer returns malformed or truncated JSON, only that chunk is recursively split and retried. A singleton receives at most `1 + review_output_retries` attempts; persistent failure remains visible and marks the chapter review as failed. Service and transport failures are not multiplied by this output-recovery path. Severe issues are only retranslated when `autofix_severe` is enabled.
