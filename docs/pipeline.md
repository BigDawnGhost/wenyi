...
- **Final review:** starts only after every chapter has been translated, so each chapter derives its relevant term snapshot from the completed glossary rather than the glossary state from an earlier chapter. Chapters are divided into contiguous chunks and checked in parallel against fixed final translation and term snapshots; results are merged back in book order. Severe issues are only retranslated when `autofix_severe` is enabled. A candidate must first pass deterministic checks for non-empty content, actual change, plausible length, and preservation of shared numbers, then receive an explicit `accept` from an independent low-cost verifier before it can replace the current translation. A failed call, malformed verdict, or uncertainty keeps the current translation unchanged and records the rejection for manual review.
