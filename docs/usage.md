...
- Character/paragraph style: bold/italic/underline/color/size, plus alignment and paragraph shading. Source Western fonts are not kept; Chinese targets use **Song (宋体)**. Uniform runs apply without a model call; mixed runs align after translate (EPUB-annotation-like markers), falling back to proportional offsets. Built-in Heading theme blue is neutralized unless the source set an explicit color.
