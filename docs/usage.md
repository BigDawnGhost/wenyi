...
run marked `running`, `interrupted`, or `failed` when translation content, review
configuration, and glossary fingerprints match. Runs are read-only by default;
`--autofix` may publish their final revisions.
