...
进入书籍项目的 **边翻边看** 页面，可按章节阅读原文和当前译文。
页面每 2 秒读取已保存的批次结果，刷新时保留滚动位置和段落选择；
“跟随本章最新译文”开启后，会在新增已译段落时滚动到该段落。
原文完成初始化后即可进入，未译段落显示“等待翻译”。

阅读页右侧的 **编辑术语表** 面板支持新增、修改、删除、导入和解决冲突。
翻译期间可保存修改，后台在下一个新翻译批次发出前读取最新术语；
已经发出的请求继续使用提交时的术语。既有译文不会因修改术语自动全部重写。
原文解析、初始化和译前准备期间需要等待准备完成。

选择一个或多个已经翻译的文本段落，点击 **重译所选段落** 即可提交独立任务。
每次最多选择 50 段，同一项目最多保留 5 个等待或执行中的重译请求；
同一段落不能重复排队。任务使用提交时的模型配置、执行时的最新术语和章节上下文，
按项目设置执行润色，产生的模型用量计入项目统计。
主翻译流程继续执行，不需要为段落重译暂停整书任务。

重译任务在现有 `export-worker` 进程中执行，与整书翻译队列独立。
部署需要同时运行 workflow worker 和 export worker（Compose `full` 配置已包含）。
任务记录保存在 PostgreSQL 的 `retranslation_requests` 表，
启动 API / Worker 时会自动创建该附加表，既有项目可以直接使用。
任务失败、中断或遇到译文版本冲突时，阅读页保留原译文并显示任务记录；
多段任务已成功写回的段落仍会保留，可重新选择剩余段落提交。

每段译文维护版本号，重译只有在原文、旧译文和版本均未变化时才会写回。
整书任务保存旧章节快照时会保留较新的重译结果；重译完成后，相关旧审校状态和
文本定位信息失效。正文或术语表在审校期间发生变化时，该次旧审校的自动修复会跳过，
避免恢复过时的译法；之后可重新运行审校。项目有活动重译请求时禁止删除。

字幕仍使用“字幕对照与编辑”页面；段落重译功能适用于书籍项目。
