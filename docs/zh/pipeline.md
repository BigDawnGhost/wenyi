...
预扫会生成逐章梗概和全书概览；超长章节按开头/中部/结尾三点采样，章末的高潮与转折同样进入译者视野。翻译每个批次时，提示词会按稳定信息优先的顺序提供角色风格、全书概览、章节梗概、相关术语表、最近译文和待译原文。

这使早期章节可以参考后续剧情，同时让同章相邻批次在代词、称谓、语气和跨段句意上保持衔接。最近译文尾巴按段数（`rolling_context_segments`）加字符预算下限（`rolling_context_min_chars`）注入：对话密集段落会自动向前多取更早译文，避免指代上下文断档。
...
- **首译长度门**：译文/原文字符比明显过短的段（典型的"只翻一半"信号）自动定向重译一次，重译仍短则保留原译，翻译正常时零额外调用；润色后异常变短的段零成本回退润色前译文；过长段只记录、不自动改写。
- **润色**：不改变原意和段落数量的前提下提升目标语言流畅度。
- **标点规范化**：中文输出统一为大陆常用全角标点；英文输出只保守清理明显残留的中日文句读。
...
每个完成批次都会立即写入状态目录。再次运行 `translate` 会跳过已有译文，仅补齐未完成部分；保存后的源语言/目标语言方向不能在原状态目录内更改；`assemble` 可以直接从状态目录重新导出成品。
