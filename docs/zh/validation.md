...
## 仓库审核与 CI 稳定性修复 · 2026-09-17

检查了待提交的 Docker 改动、API 鉴权与请求处理、任务恢复、导出保留策略和过时／生成
代码引用。本次是重点源码审核加全仓自动化检查，不代表证明所有执行路径都没有缺陷。

- 修复启用 token 后跨域预检先被鉴权拦截的问题；回归用例在修复前返回 HTTP 401，
  修复后覆盖允许／拒绝的来源、受保护请求及鉴权错误的 CORS 响应头。
- 删除重复的 OpenAPI 覆盖逻辑，停止跟踪 TypeScript 工具编译产物。Docker Worker
  共用 API 依赖镜像，Buildx 下载缓存仅在显式选择时启用。
- 修复人工校阅 CI 时序问题：保存并刷新后等待编辑窗口关闭，再定位已保存段落，
  避免全页面文本匹配同时命中译文和编辑框。
- Python 3.12：**956 passed、3 skipped、49 subtests passed**，包含隔离 PostgreSQL
  和 Redis 测试。3 项跳过均因缺少可选依赖 `fpdf2`。
- 最终完整 Playwright：**59 passed**；修复的人工校阅用例连续 **12 次通过**。
  中间一次全量运行出现字幕导航加载失败；该用例独立重复 6 次及最终全量运行均通过，
  未修改该用例，根因尚未确认。
- TypeScript 检查（含未使用变量／参数）、Vite 构建、Ruff check/format、修改的 E2E
  格式、Dockerfile shell 语法、Compose 覆盖一致性和 Git 空白检查均通过。
- Docker 实际构建在 apt 下载长时间停滞后停止，**镜像构建尚未验证通过**。未调用真实
  模型或 PDF 服务，未重跑 Python 3.10 和发布包构建。仍有 Arq 弃用与 Vite 包体积警告。

临时数据库和队列容器已移除；运行中的部署服务与本地书籍／状态未修改。用户的图片改动
不纳入这些提交。
