...
class TestTierResolution(unittest.TestCase):
    ...
    def test_tier_can_override_json_response_fallback(self):
        cfg = _config(
            json_response_fallback="none",
            tiers={
                "strong": TierConfig(
        ...
        self.assertEqual(
            resolve_request_config(cfg, "strong").json_response_fallback,
            "reasoning_content",
        )
        self.assertEqual(
            resolve_request_config(cfg, "cheap").json_response_fallback,
            "none",
        )
...
class TestSDKAndCompletion(unittest.TestCase):
    ...
    def test_openai_empty_response_retries_then_returns_text(self):
        ...
        create = MagicMock(side_effect=[empty, valid])
        client._client = SimpleNamespace(
            chat=SimpleNamespace(completions=SimpleNamespace(create=create))
        )

        # Retry waits are transport policy, not the behavior under test here.
        with patch("trans_novel.llm.retrying._FALLBACK_WAIT", return_value=0.0):
            self.assertEqual(
    ...
    def test_anthropic_empty_response_retries_then_returns_text(self):
        client = UniversalClient(_config(api_format="anthropic", max_retries=1))
        empty = SimpleNamespace(content=[], usage=None, stop_reason=None)
        valid = SimpleNamespace(
        ...
        # The same retry contract must apply to both supported API formats.
        with patch("trans_novel.llm.retrying._FALLBACK_WAIT", return_value=0.0):
            self.assertEqual(
                client.complete([{"role": "user", "content": "x"}]),
                "done",
    ...
    def test_anthropic_truncation_is_not_treated_as_retryable_empty_output(self):
        client = UniversalClient(_config(api_format="anthropic", max_retries=1))
        response = SimpleNamespace(content=[], usage=None, stop_reason="max_tokens")
        create = MagicMock(return_value=response)
        client._client = SimpleNamespace(messages=SimpleNamespace(create=create))
    ...
    def test_empty_response_exhaustion_reraises_domain_error(self):
