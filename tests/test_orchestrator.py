...
class TestReviewReporting(unittest.TestCase):
    ...
    def _handler(self, fix_text, verifier_response=None):
        ...
        def handler(messages, tier, json_mode):
            ...
            if "中文润色编辑" in sys:
                ...
                return json.dumps(
                    {"polished": [self.CURRENT_TEXT for _ in range(count)]},
                    ensure_ascii=False,
                )
    ...
    def test_autofix_adopts_retranslation(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            applied = [event for event in events if event.get("event") == "autofix_applied"]
    ...
    def test_autofix_rejects_short_retranslation(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            verifier_calls = [
                call for call in client.calls if "修复验证员" in call["messages"][0]["content"]
            ]
    ...
    def test_autofix_verifier_rejects_candidate_and_records_reason(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(all(i.get("fixed") is False for i in flagged))
            self.assertTrue(
                all(i["repair_verification"]["code"] == "model_rejected" for i in flagged)
            )
            self.assertNotEqual(ch.text_segments[0].target, self.FIX_TEXT)
            with open(store.event_log_path, encoding="utf-8") as file:
                events = [json.loads(line) for line in file]
            rejected = [event for event in events if event.get("event") == "autofix_rejected"]
    ...
    def test_autofix_verifier_failure_keeps_original(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(all(i.get("fixed") is False for i in flagged))
            self.assertTrue(
                all(i["repair_verification"]["code"] == "invalid_response" for i in flagged)
            )
            self.assertNotEqual(ch.text_segments[0].target, self.FIX_TEXT)
