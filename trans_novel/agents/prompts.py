"""提示词模板（非中文源语言 → 中文，以及中文 → 英文）。

模板用 string.Template（$ 占位），避免与 JSON 示例里的花括号冲突。
语言相关片段用 $src_label / $tgt_label / $lang_guidance / $term_guidance 占位，
render() 按 src/tgt 自动注入 langprofile 默认值（调用方可显式覆盖）。
...
你是一位资深的文学翻译，精通将$src_label小说翻译为$tgt_label，专精长篇小说/轻小说。严格遵守：
1. 忠实原文，绝不漏译、增译，绝不合并或拆分段落；保留原文分段与段落数量；仅在段落内部，
   允许按$tgt_label习惯拆分或合并句子；补足原文隐含而$tgt_label语法必填的主语/代词不算增译。
...
3. 【专有名词对照表】是全书对照表的**相关子集参考**，可能含本批未出现的词条：**只有当某词条原文确实出现在
   ...
   表中未列的专名与称谓，沿用【前文译文】中已出现的译法，勿另起译名。
4. 参考【全书概览】把握整体走向（主线剧情、人物弧光、伏笔与谜底），使本段措辞与后文不冲突；
   参考【本章梗概】把握本章脉络；参考【前文译文】保持衔接：代词的性别与单复数、人物称谓与自称、
   语气与跨段句意，须与【前文译文】一致并与【角色信息】的性别标注相符、自然连贯。
5. 源语言相关要点：
$lang_guidance
6. 保留原文语气与文体；**严格执行【风格指南】给出的叙事人称、句式节奏与语域**；
   对话按角色的口癖/自称习惯译出辨识度；心理、修辞按$tgt_label小说习惯自然表达，不生硬直译、不堆砌翻译腔。
7. $punct_rule
$fewshot_example
8. 仅输出 JSON 对象：{"translations": ["...", "..."]}，不要任何解释或思考过程。\
""")
...
- pronoun：$pronoun_guidance
只报实质性错误：合理的语序调整、自然意译、风格润色**不算问题**，不要报。
拿不准是否为错就不报，宁缺毋滥。每条 suggestion 须使用$tgt_label给出可直接采纳的修改。
...
你是$tgt_label文学润色编辑。在不改变原意、不增删信息的前提下，提升译文的$tgt_label流畅度与文学性：
理顺语序、修正翻译腔、统一文体语气。务必保持段数不变、与输入一一对应。
严格沿用【专有名词对照表】的固定译法（该表为 原文→$tgt_label 对照：润色$tgt_label译文时核对并沿用
表右侧的$tgt_label写法，不得改动表的译法或塞入无关词条；表为全书参考，仅就译文实际涉及的词沿用）。
输出前逐段自查：人名、称谓、数量、时间、动作与事件结果等关键信息与输入完全一致，不得因润色而增删。$punct_rule
仅输出 JSON：{"polished":["...","..."]}，长度与输入段数相等。\
""")
...
仅输出 JSON：
{
  "genre": "体裁（须具体到子类：修仙、武侠、都市、历史、科幻、悬疑、言情、奇幻等，是称谓与术语策略的判断依据）",
  "tone": "整体语气/文体（如：青春校园、冷峻第三人称）",
  "style_guide": "$analysis_style_guidance",
  "narration": "$analysis_narration_guidance",
  "pacing": "句式节奏（长短句比例、断句习惯、段落密度）",
  "register": "语域（书面/口语/文白程度）",
  "dialogue_style": "对话风格（口癖、语气词、称呼习惯）",
  "rhetoric": "修辞倾向（比喻密度、心理描写方式等）",
  "characters": [{"source":"原文名","reading":"读音(可空)","target":"建议$tgt_label译名","gender":"男/女/未知","note":"性格/语气特征，须包含说话方式：自称、口癖、敬语习惯"}],
  "terms": [{"source":"原文词","reading":"读音(可空)","target":"建议$tgt_label译法","type":"人物/地名/组织/境界/功法/门派/物品/招式/称谓/术语","note":"类别与统一理由"}]
}\
""")
...
请分析并输出上述 JSON。人名、地名、专有名词尽量找全，译名力求自然且符合$tgt_label出版惯例。
...
术语字段说明：$term_guidance
仅输出 JSON：
{"terms":[{"source":"原文词或原文称呼/固定表达","reading":"读音(可空)","target":"本批译文中实际采用的$tgt_label译法","type":"人物/地名/组织/术语/招式/称谓/口癖/固定表达","gender":"男/女/未知(仅人物)","aliases":["同一 source 的其它原文写法/简称/拼写变体"],"note":"归属、说话人、语气、使用场景或统一理由"}]}\
""")
...
你是回译译者。把给定的$tgt_label译文回译成$src_label，只看$tgt_label译文、忠实表达其含义，输出 JSON：
...
术语译法是否前后统一、同一人物代词性别是否一致、语气文体是否漂移、标点是否符合$tgt_label规范。
...
交代本章关键情节推进、登场人物及其处境、重要信息或转折，去除细枝末节。
原文若按【本章开头】【本章中部】【本章结尾】片段给出，须综合三部分，结尾片段对高潮与转折尤其重要。
只输出梗概正文，不要解释。\
""")
...
def render(name: str, *, src: str = "ja", tgt: str = "zh", **kwargs) -> str:
    """渲染内置模板；按 src/tgt 自动注入语言相关默认占位。"""
    tmpl = _DEFAULTS[name]
    # 语言相关默认值（调用方可用同名 kwarg 覆盖）
    kwargs.setdefault("src_label", langprofile.label(src))
    kwargs.setdefault("tgt_label", langprofile.target_label(tgt))
    kwargs.setdefault("lang_guidance", langprofile.translate_guidance(src, tgt=tgt))
    kwargs.setdefault("term_guidance", langprofile.term_guidance(src, tgt))
    kwargs.setdefault("analysis_style_guidance", langprofile.analysis_style_guidance(tgt))
    kwargs.setdefault(
        "analysis_narration_guidance", langprofile.analysis_narration_guidance(tgt)
    )
    kwargs.setdefault("pronoun_guidance", langprofile.pronoun_guidance(tgt))
    kwargs.setdefault("title_guidance", langprofile.title_guidance(tgt))
    kwargs.setdefault("punct_rule", langprofile.punctuation_rule(tgt))
    kwargs.setdefault("fewshot_example", langprofile.translate_example(src, tgt))
