...
def usage_delta(current: dict[str, Any], previous: dict[str, Any]) -> dict[str, Any]:
    ...
    tier_delta = _usage_group_delta(
        current["by_tier"], previous["by_tier"]
    )
    stage_delta = _usage_group_delta(
        current["by_stage"], previous["by_stage"]
    )
...
def merge_usage_summaries(
    ...
    tiers = _merge_usage_groups(
        accumulated["by_tier"], increment["by_tier"]
    )
    stages = _merge_usage_groups(
        accumulated["by_stage"], increment["by_stage"]
    )
