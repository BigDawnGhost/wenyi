...
def extract_openai_json_fallback(response: Any) -> str | None:
    """严格读取非标准 reasoning_content；混合文本或残缺 JSON 一律拒绝。"""
    choices = getattr(response, "choices", None)
    if not choices:
        raise RuntimeError("OpenAI 格式 API 未返回 choices")
    ...
    # json.loads 会拒绝 JSON 前后的说明文字，防止把推理过程误当最终答案。
    try:
        json.loads(fallback)
    except json.JSONDecodeError as error:
        raise EmptyResponseError("OpenAI 配置的 JSON 备用响应不是合法 JSON") from error
    return fallback
        ...
        with self.request_activity(stage=stage, tier=tier) as request_id:
            ...
            def _call() -> str:
                if self.api_format == "openai":
                    ...
                    # 空 content 只有在 JSON 模式且显式允许时才读取厂商扩展字段。
                    if not text.strip():
                        choice = response.choices[0]
                        if str(getattr(choice, "finish_reason", "")).lower() == "length":
                            raise RuntimeError("OpenAI 格式 API 响应因达到 token 上限而截断")
                        if json_mode and settings.json_response_fallback == "reasoning_content":
                            text = extract_openai_json_fallback(response) or ""
                else:
                    response = client.messages.create(**kwargs)
                    sample = normalize_anthropic_usage(getattr(response, "usage", None))
                    text = extract_anthropic_text(response)

                    # Anthropic 达到输出上限时不可按普通空响应重试，以免隐藏配置错误。
                    if (
                        not text.strip()
                        and str(getattr(response, "stop_reason", "")).lower() == "max_tokens"
                    ):
                        raise RuntimeError("Anthropic API 响应因达到 token 上限而截断")
                self.usage.record(tier, sample, stage)
                if not text.strip():
                    field = "content" if self.api_format == "openai" else "text"
                    raise EmptyResponseError(f"{self.provider_name} 响应的 {field} 为空")
                return text
