...
from ..agents.reviewer import BackTranslator, RepairVerifier, Reviewer, ReviewOutputError
...
class Orchestrator:
    ...
    def _autofix_severe(
        ...
        *,
        ...
        候选先过非空、实质变化、同目标语言长度和数字保留等确定性门禁，
        再交给独立修复验证员比较原文、旧译文、候选和上下文。只有明确
        accept 才写回；任何调用、格式或判断异常均保留旧译文并留人工。
        最终审校重译时原滚动上下文已失效，用该段前后各 2 段译文做局部上下文。
        """
