...
定位内存章节、Artifact 或未来 LangGraph 节点的输入切片。
"""
...
def plan_contiguous_batches(
    ...
    budget = _validate_budget(max_chars)
    facts = _segment_facts(segments)
    return _contiguous_plans(facts, budget)
...
def plan_resumable_batches(
    ...
    budget = _validate_budget(max_chars)
    facts = _segment_facts(segments)
    raw_plans = _contiguous_plans(facts, budget)
